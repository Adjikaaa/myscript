...
#	print(d)
	...
	for key in d.keys():
		...
		for cnfname in d.get(key):
			...
			print(configuration)
			#создаем массив массивов, куда будем перезаписывать каждую строку конфигурации (она в формате строки) в массив интов 
			res = []
			#
			for k in range(len(configuration)):
				
				#
				for elem in configuration[k]:
				...
				for i in range(len(res)):
					...
					for j in range(len(res)):
						if res[i] not in invers and res[j] == temp and i != j:
							...
							invers.append(temp)
							cntinv += 1
				if (list(configuration[k])).index(elem) > 0:
